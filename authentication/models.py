...
#     def __str__(self):
#         return f"{self.student.name} - {self.date} - {'Present' if self.attended else 'Absent'}"
class Attendance(models.Model):
    student = models.ForeignKey(TblStudents, on_delete=models.CASCADE)
    datetime = models.DateTimeField(auto_now_add=True)  # Update to store both date and time
    attended = models.BooleanField(default=False)
